...
def main():
    ...
    axes[1].errorbar(X[1], Y[1], yerr=np.sqrt(avg_latency_sigma/10), color='blue', alpha=0.1)
    axes[1].plot(X[1], Y[1], color='purple')
    
    #'CpuUtilization (%)'
    axes[1].set(ylabel='Average Latency (usec)', xlabel='Multiple')
